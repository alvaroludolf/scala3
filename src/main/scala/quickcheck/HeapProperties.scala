...
trait HeapProperties(val heapInterface: HeapInterface):
  ...
  val meldingSmallHeaps: (String, Prop) =
    "melding a heap containing two low values with a heap containing two high values" ->
      forAll { (x: Int, y: Int) =>
        ...
        val heap1 = insert(max, insert(max, empty))
        val heap2 = insert(min, insert(min, empty))
        var meldedHeap: List[Node] = meld(heap1, heap2)
        ...
        // highest value
        meldedHeap = deleteMin(deleteMin(meldedHeap))
        val deleteTwoMinAndFindMin: Boolean = max == findMin(meldedHeap)
        // check that inserting the lowest value to the melded heap, and then
        // finding the minimal element returns the lowest value
        meldedHeap = insert(min, meldedHeap)
        val insertMinAndFindMin: Boolean = min == findMin(meldedHeap)
  ...
  val meldingHeaps: (String, Prop) =
    def check(heap1: List[Node], heap2: List[Node], heapm: List[Node]): Boolean =
      if !isEmpty(heap1) then
        if !isEmpty(heap2) then
          val minm: Int = findMin(heapm)
          val min1: Int = findMin(heap1)
          val min2: Int = findMin(heap2)
          if (min1 <= min2) then
            minm == min1 && check(deleteMin(heap1), heap2, deleteMin(heapm))
          else
            minm == min2 && check(heap1, deleteMin(heap2), deleteMin(heapm))
        else
          val minm: Int = findMin(heapm)
          val min1: Int = findMin(heap1)
          minm == min1 && check(deleteMin(heap1), heap2, deleteMin(heapm))
    ...
    forAll { (heap1: List[Node], heap2: List[Node]) =>
      val heapm = meld(heap1, heap2)
      check(heap1, heap2, heapm)
    }
