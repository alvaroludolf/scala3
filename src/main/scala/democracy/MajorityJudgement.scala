...
case class Election(description: String, candidates: Set[Candidate]):
  ...
  def elect(ballots: Seq[Ballot]): Candidate =
    ...
    ballots.flatMap(ballot => ballot.grades.toList)
    ...
    allGrades.groupMap(key => key._1)(f => f._2)
  ...
  // is chosen by lottery from among the candidates.
    if gradesPerCandidate.forall((_, grades) => grades.isEmpty) then
      ...
      // of the collection `bestCandidates`
        ...
        bestCandidates.map((candidate: Candidate, grades: Seq[Grade]) => (candidate, grades.diff(List(bestMedianGrade))))
